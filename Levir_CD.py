...
root = r'F:\PingAn\NMG\22_27\output\ceshi'
...
def rand_crop_CD(img1, mask, contour, LULC, size):
    # print(img.shape)
    h = img1.shape[0]
    w = img1.shape[1]
    c_h = size[0]
    ...
    if h < c_h or w < c_w:
        ...
        lulc_label = LULC[s_h:e_h, s_w:e_w]
        # print('%d %d %d %d'%(s_h, e_h, s_w, e_w))
        return crop_im1, mask_im2, contour_label, lulc_label

def rand_flip_CD(img1, mask, contour, lulc):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img1, mask, contour, lulc
    elif r < 0.5:
        return np.flip(img1, axis=0).copy(), np.flip(mask, axis=0).copy(), np.flip(contour, axis=0).copy(), np.flip(lulc, axis=0).copy()
    elif r < 0.75:
        return np.flip(img1, axis=1).copy(), np.flip(mask, axis=1).copy(), np.flip(contour, axis=1).copy(), np.flip(lulc, axis=1).copy()
    else:
        return img1[::-1, ::-1, :].copy(), mask[::-1, ::-1].copy(), contour[::-1, ::-1].copy(), lulc[::-1, ::-1].copy()

def read_RSimages(mode, read_list=False):
    assert mode in ['train', 'val', 'test']
    img_A_dir = os.path.join(root, mode, 'train_image')
    mask_dir = os.path.join(root, mode, 'train_mask')
    contour_dir = os.path.join(root, mode, 'train_boundary')
    LULC_dir = os.path.join(root, mode, 'train_dist')
    ...
    data_A, mask_B, contour_labels, LULC_lables = [], [], [], []
    for idx, it in enumerate(data_list):
        if (it[-4:]=='.tif'):
            ...
            LULC_path = os.path.join(LULC_dir, it)
...
class RS(data.Dataset):
    def __init__(self, mode, random_crop=False, crop_nums=6, sliding_crop=False, crop_size=256, random_flip=True):
        ...
        data_A, mask, contour, lulc = read_RSimages(mode, read_list=False)
        if sliding_crop:
            data_A, mask, contour = sliding_crop_CD(data_A, mask, contour, [self.crop_size, self.crop_size])
        self.data_A, self.masks, self.contours, self.LULC = data_A, mask, contour, lulc
        if self.random_crop:
    ...
    def __getitem__(self, idx):
        ...
        LUCLS = self.LULC[idx]
        if self.random_crop:
            data_A, mask, boundary, LUCLS = rand_crop_CD(data_A, mask, boundary, LUCLS, [self.crop_size, self.crop_size])
        if self.random_flip:
            data_A,  mask, boundary, LUCLS = rand_flip_CD(data_A, mask, boundary, LUCLS)
        return F.to_tensor(data_A),  mask, boundary, LUCLS
